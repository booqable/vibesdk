...
interface BaseAgentInitArgs {
    ...
    /**
     * Host-supplied signed session token. Think appends it to the preview URL
     * its `get_browser_console_logs` tool loads, so the agent debugs the app
     * authenticated (against real host API data) instead of a logged-out shell.
     */
    previewToken?: string;
}
