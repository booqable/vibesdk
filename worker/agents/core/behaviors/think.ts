...
export class ThinkCodingBehavior
	...
	private buildSystemPrompt(modelName: string, provider: string): string {
		return [
			...
			'',
			'## Build directly — clarification already happened',
			"The request below already incorporates the user's answers to a clarifying questionnaire that ran BEFORE this build (the host gathers those up front). Do NOT ask further clarifying questions and do NOT pause for input — there is no channel to answer them mid-build, so doing so stalls the build. Proceed straight to building. Where a detail is still open, choose sensible, production-quality defaults and note them briefly as you go.",
			'',
